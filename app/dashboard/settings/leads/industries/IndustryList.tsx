...
const industriesQueryKey = (tenantId: string, q = "") => ["leads", "industries", tenantId, q] as const;

/* -------------------------
 * API
 * ------------------------ */
const fetchIndustriesPage = async ({
  pageParam = 1,
  q = "",
  tenantId,
}: {
  ...
  q?: string;
  tenantId?: string;
}): Promise<PagedResp<Industry>> => {
  const res = await apiClient.get(`/leads/industries`, {
 ...
 * Component
 * ------------------------ */
export default function IndustryList(): JSX.Element {
  ...
  const infiniteQueryOptions: UseInfiniteQueryOptions<
    PagedResp<Industry>,
    Error,
    InfiniteData<PagedResp<Industry>>,
    readonly unknown[],
    number
  > = {
    queryKey: industriesQueryKey(tenantId, qDebounced),
    queryFn: async ({ pageParam = 1 }: { pageParam: number }) =>
      fetchIndustriesPage({ pageParam, q: qDebounced, tenantId }),
  ...
  const {
    ...
    refetch,
    isFetching,
  } = useInfiniteQuery<PagedResp<Industry>, Error, InfiniteData<PagedResp<Industry>>, readonly unknown[], number>(
    infiniteQueryOptions
  );

  const flattened = useMemo<Industry[]>(() => data?.pages.flatMap((p) => p.data.rows) ?? [], [data]);
  ...
  type InfiniteCache = {
    pages: PagedResp<Industry>[];
    pageParams: number[];
  };

  const deleteMut = useMutation({
    mutationFn: async (id: string) => {
      await apiClient.delete(`/leads/industries/${id}`, { headers: { "x-tenant-id": tenantId } });
    },
    onMutate: async (id: string) => {
      await qc.cancelQueries({ predicate: (q) => Array.isArray(q.queryKey) && q.queryKey[0] === "leads" && q.queryKey[1] === "industries" });
      const previous = qc.getQueryData<InfiniteCache>(industriesQueryKey(tenantId, qDebounced));
      qc.setQueryData<InfiniteCache>(industriesQueryKey(tenantId, qDebounced), (old) => {
    ...
    onError: (err: any, _id, ctx: any) => {
      qc.setQueryData(industriesQueryKey(tenantId, qDebounced), ctx?.previous);
            ...
            try {
              try {
                await apiClient.post(
                  `/leads/industries/${deleted.id}/restore`,
                  {},
                  { headers: { "x-tenant-id": tenantId } }
                );
              } catch {
                const recreate = { ...deleted } as any;
                delete recreate.id;
                await apiClient.post(`/leads/industries`, recreate, { headers: { "x-tenant-id": tenantId } });
              }
              ...
              toast({
                title: "Undo failed",
                description: "Could not restore industry.",
                variant: "destructive",
              });
            }
          }}
  ...
  const handleRefresh = useCallback(async () => {
    setRefreshing(true);
    const key = industriesQueryKey(tenantId, qDebounced);
    ...
    try {
      ...
      const page: PagedResp<Industry> =
        res.data && Array.isArray(res.data.rows ? res.data.rows : res.data)
          ...
          : await fetchIndustriesPage({ pageParam: 1, q: qDebounced, tenantId });
      ...
      toast({
        ...
        description: "Could not refresh industries. Check network or server.",
        variant: "destructive",
      });
    } finally {
      setRefreshing(false);
  ...
  const handleResetSearch = useCallback(() => {
    ...
    setQ("");
    setQDebounced("");
    const key = industriesQueryKey(tenantId, "");
    (async () => {
      try {
        const res = await apiClient.get("/leads/industries", {
        ...
        const page: PagedResp<Industry> =
          res.data && Array.isArray(res.data.rows ? res.data.rows : res.data)
            ...
            : await fetchIndustriesPage({ pageParam: 1, q: "", tenantId });
  ...
  const onEdit = useCallback((item: Industry) => {
    setEditing(item);
    setOpen(true);
  }, []);
  ...
  return (
    <div className="rounded-2xl bg-gradient-to-b from-slate-900/80 to-slate-950/80 text-slate-100 border border-white/10 p-6 shadow-lg backdrop-blur-xl">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-3 mb-6">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            variant="ghost"
            size="icon"
            aria-label={isFetching || refreshing ? "Refreshing industries" : "Refresh industries"}
            className="hover:bg-white/6"
      ...
      {isLoading ? (
        ...
        <div className="text-rose-400">Failed to load industries. Try the refresh button above.</div>
      ) : (
        <Virtuoso
          ref={virtuosoRef}
          ...
          itemContent={(index) => {
            ...
            return (
              <div key={item?.id ?? `i-${index}`} style={style}>
