...
const professionsQueryKey = (tenantId: string, q = "") => ["leads", "professions", tenantId, q] as const;

/* -------------------------
 * API
 * ------------------------ */
const fetchProfessionsPage = async ({
  pageParam = 1,
  q = "",
  tenantId,
}: {
  ...
  q?: string;
  tenantId?: string;
}): Promise<PagedResp<Profession>> => {
  const res = await apiClient.get(`/leads/professions`, {
  ...
  return (
    ...
    >
      <div className="flex-1">
        ...
        {item.category && <div className="text-xs text-slate-400 mt-1">Category: {item.category}</div>}
 ...
 * Component
 * ------------------------ */
export default function ProfessionList(): JSX.Element {
  ...
  const infiniteQueryOptions: UseInfiniteQueryOptions<
    PagedResp<Profession>,
    Error,
    InfiniteData<PagedResp<Profession>>,
    readonly unknown[],
    number
  > = {
    queryKey: professionsQueryKey(tenantId, qDebounced),
    queryFn: async ({ pageParam = 1 }: { pageParam: number }) =>
      fetchProfessionsPage({ pageParam, q: qDebounced, tenantId }),
  ...
  const flattened = useMemo<Profession[]>(() => data?.pages.flatMap((p) => p.data.rows) ?? [], [data]);
  ...
  type InfiniteCache = {
    pages: PagedResp<Profession>[];
    pageParams: number[];
  };

  const deleteMut = useMutation({
    mutationFn: async (id: string) => {
      await apiClient.delete(`/leads/professions/${id}`, { headers: { "x-tenant-id": tenantId } });
    },
    onMutate: async (id: string) => {
      // cancel and optimistic remove
      await qc.cancelQueries({ predicate: (q) => Array.isArray(q.queryKey) && q.queryKey[0] === "leads" && q.queryKey[1] === "professions" });
      const previous = qc.getQueryData<InfiniteCache>(professionsQueryKey(tenantId, qDebounced));
      qc.setQueryData<InfiniteCache>(professionsQueryKey(tenantId, qDebounced), (old) => {
    ...
    onError: (err: any, _id, ctx: any) => {
      qc.setQueryData(professionsQueryKey(tenantId, qDebounced), ctx?.previous);
    ...
    // use predicate invalidation (type-safe)
    onSettled: async () => {
      await qc.invalidateQueries({
        predicate: (query) =>
          Array.isArray(query.queryKey) &&
          query.queryKey[0] === "leads" &&
          query.queryKey[1] === "professions" &&
            ...
            try {
              try {
                await apiClient.post(
                  `/leads/professions/${deleted.id}/restore`,
                  {},
                ...
                await apiClient.post(`/leads/professions`, recreate, { headers: { "x-tenant-id": tenantId } });
              }

              // Invalidate queries for this tenant + current search to refresh UI
              await qc.invalidateQueries({
                predicate: (query) =>
                  Array.isArray(query.queryKey) &&
                  query.queryKey[0] === "leads" &&
              ...
              toast({
                title: "Undo failed",
                description: "Could not restore profession.",
                variant: "destructive",
              });
            }
          }}
  ...
  const handleRefresh = useCallback(async () => {
    setRefreshing(true);
    const key = professionsQueryKey(tenantId, qDebounced);
    ...
    try {
      ...
      const page: PagedResp<Profession> =
        res.data && Array.isArray(res.data.rows ? res.data.rows : res.data)
          ...
          : await fetchProfessionsPage({ pageParam: 1, q: qDebounced, tenantId });
      ...
      toast({
        ...
        description: "Could not refresh professions. Check network or server.",
        variant: "destructive",
      });
    } finally {
      setRefreshing(false);
  ...
  const handleResetSearch = useCallback(() => {
    ...
    setQ("");
    setQDebounced("");
    const key = professionsQueryKey(tenantId, "");
    (async () => {
      try {
        const res = await apiClient.get("/leads/professions", {
        ...
        const page: PagedResp<Profession> =
          res.data && Array.isArray(res.data.rows ? res.data.rows : res.data)
            ...
            : await fetchProfessionsPage({ pageParam: 1, q: "", tenantId });
  ...
  const onEdit = useCallback((item: Profession) => {
    setEditing(item);
    setOpen(true);
  }, []);

  const onDelete = useCallback((id: string) => {
    if (!confirm("Delete profession?")) return;
    deleteMut.mutate(id);
  }, [deleteMut]);
    ...
    <div className="rounded-2xl bg-gradient-to-b from-slate-900/80 to-slate-950/80 text-slate-100 border border-white/10 p-6 shadow-lg backdrop-blur-xl">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-3 mb-6">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            variant="ghost"
            size="icon"
            aria-label={isFetching || refreshing ? "Refreshing professions" : "Refresh professions"}
            className="hover:bg-white/6"
      ...
      {isLoading ? (
        <div className="space-y-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={`skeleton-${i}`} className="p-4 border border-white/10 bg-white/4 rounded-xl animate-pulse">
              <div className="h-4 w-1/3 bg-slate-700 rounded mb-2" />
        ...
        <div className="text-rose-400">Failed to load professions. Try the refresh button above.</div>
      ) : (
        <Virtuoso
          ref={virtuosoRef}
      ...
      <ProfessionForm
        ...
        onClose={async () => {
          setOpen(false);
          setEditing(null);
          // invalidate by tenant (type-safe)
          await qc.invalidateQueries({
            predicate: (query) =>
              Array.isArray(query.queryKey) &&
              query.queryKey[0] === "leads" &&
