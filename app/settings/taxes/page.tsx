...
// Full Company Tax Admin + Preview Page (defensive)
...
function ensureArray<T>(v: any): T[] {
  ...
  try {
    ...
    if (v.companies && Array.isArray(v.companies)) return v.companies;
    // single entity -> wrap
    if (v && typeof v === "object" && v.id && (v.name || v.id)) return [v] as any;
...
export default function Page() {
  ...
  async function loadCompanyTaxMaps(cid: string) {
    ...
    try {
      ...
      // normalize to array (allow single object)
  ...
  async function assignTaxToCompany(taxRateId: string) {
    ...
    try {
      ...
      if (!res.ok) {
        console.error("[assignTaxToCompany] non-2xx body:", json);
        throw new Error(`Assign failed: ${res.status} ${res.statusText} - ${typeof json === "object" ? JSON.stringify(json) : json}`);
      }
  ...
  async function updateCompanyTaxMap(id: string, patch: Partial<CompanyTaxMap>) {
    ...
    try {
      ...
      try { json = txt ? JSON.parse(txt) : null; } catch { json = txt; }
      if (!res.ok) {
        console.error("[updateCompanyTaxMap] non-2xx body:", json);
        throw new Error(`Update failed: ${res.status} ${res.statusText} - ${typeof json === "object" ? JSON.stringify(json) : json}`);
      }
      await loadCompanyTaxMaps(companyId);
    } catch (err: any) {
  ...
  async function previewTaxes() {
    ...
    try {
      ...
      if (!res.ok) {
        console.error("[previewTaxes] non-2xx body:", json);
        throw new Error(`Preview request failed: ${res.status} ${res.statusText} - ${typeof json === "object" ? JSON.stringify(json) : json}`);
      }
                ...
                <div className="p-4 rounded-xl bg-white/40">
                  ...
                  {loading ? <div>Loading…</div> : (
                    <div className="space-y-2">
                      {mapList.length === 0 && <div className="text-sm text-muted">No taxes assigned to this company.</div>}
